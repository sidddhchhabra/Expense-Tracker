...
# RUN COMMAND "pip install tkcalendar" in CMD to install tkcalender in the system.
...
#GUI
root = Tk()
root.title('EXPENSE TRACKER by SIDDH CHHABRA')
root.iconbitmap(r'C:\Users\INDIA\Desktop\TrngPrjct\Calculator_31111.ico')
...
# Data Entry
Label(data_entry_frame, text='Date (DD/MM/YYY) :', font=lbl_font, bg=dataentry_frame_bg).place(x=10, y=50)
date = DateEntry(data_entry_frame, date=datetime.datetime.now().date(), font=entry_font, date_pattern='dd/mm/yyy')
...
# Buttons
Button(buttons_frame, text='Delete Expense', font=btn_font, width=25, bg=hlb_btn_bg, command=delete_expense).place(x=30, y=5)
...
Button(buttons_frame, text='Delete All Expenses', font=btn_font, width=25, bg=hlb_btn_bg, command=delete_all_expenses).place(x=640, y=5)

Button(buttons_frame, text='View Selected Expense\'s Details', font=btn_font, width=25, bg=hlb_btn_bg,
       command=view_selected_expense).place(x=30, y=65)
...
table.heading('ID', text='Exp ID', anchor=CENTER)
